...
while True:
    ...
    # Não é possível a utilização 
    '''# CPU TEMP 
    cpu_temp_tui = cpu_tui.items[9]
    ps_cpu_temp = sensors_temperatures()['coretemp'][1]
    ...
    ps_pch_temp = sensors_temperatures()['pch_skylake'][1]
    ...
    outros_tui.text = f'Bateria: {sensors_battery()} %' # No computador não é fornecido este dado sendo assim, ele retorna com none.
    ...
    # Erro com o nome das minhas partições de disco
...
'''
    ...
    addrs_V4 = net_if_addrs()['Ethernet'][0]
    addrs_V6 = net_if_addrs()['Ethernet'][1]
    network_tui.text = f'IPV4: {addrs_V4.address}\n'
    network_tui.text += f'MaskV4: {addrs_V4.netmask}\n'
    network_tui.text += f'IPV6: {addrs_V6.address}\n'
